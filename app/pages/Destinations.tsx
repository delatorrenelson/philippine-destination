...
export default function Destinations() {
  ...
  useEffect(() => {
    fetch("/api/destinations")
      ...
      .catch((err) => console.error("Error fetching destinations from MongoDB API:", err))
  ...
  const filtered = destinations.filter((dest) => {
    const matchesSearch =
      ...
      (dest.region && dest.region.toLowerCase().includes(search.toLowerCase())) ||
      (dest.islandGroup && dest.islandGroup.toLowerCase().includes(search.toLowerCase()));

    const matchesRegion =
      selectedRegion === "All" ||
      (dest.region && dest.region.toLowerCase().includes(selectedRegion.toLowerCase()));
    ...
    <div className="py-8 max-w-7xl mx-auto px-4 sm:px-6 space-y-8">
        ...
        <div className="pt-4 flex flex-col sm:flex-row gap-3 max-w-xl">
          <div className="relative flex-1">
            <input
              ...
              placeholder="Search destination, island, region..."
      ...
      {loading ? (
        <div className="py-12 text-center text-xs text-gray-400 font-medium">
          Loading destinations from MongoDB...
        </div>
      ) : (
          ...
          {filtered.map((item) => (
            ...
            >
              <div className="relative aspect-[16/10] overflow-hidden bg-gray-100 dark:bg-gray-800">
                <img
                  src={item.heroImage || item.image || FALLBACK_IMAGE}
                  alt={item.name}
                ...
                <span className="absolute top-3 left-3 bg-emerald-600/90 backdrop-blur-md text-white text-[11px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider">
                  {item.islandGroup || item.category || "Philippines"}
                </span>
              ...
              <div className="p-5 flex-1 flex flex-col justify-between space-y-3">
                ...
                <div className="pt-3 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between text-xs">
                  <span className="text-[11px] text-gray-400">
                    Spot: {item.topAttractions?.[0]?.name || item.popularSpots?.[0] || "Island Attraction"}
                  </span>
                  <Link
                    to={`/booking/${item.name.toLowerCase().replace(/\s+/g, "-")}`}
